...
export default function Navbar(props : allItemsRef) {
  ...
  return (
    <header className="w-8/12 flex flex-col items-center text-zinc-800 dark:text-zinc-300 fixed right-16 z-50 font-syne">
      <div className="flex justify-between items-center w-full">
        <ul className=" flex items-center justify-between gap-x-2 text-xl font-bold dark:[&>*]:bg-black  [&>*]:item-nav [&>*]:bg-slate-200  [&>*]:shadow-md ">
          {data.navBarItems.map(item=>
              ...
              className="transition-all duration-500 ease-in-out">{item.navItem}</li>
            )}
        </ul>
        <div className=" h-max  w-3/12 flex items-center justify-center  ">
          ...
          <DefaultButton isLinkTag={true} href={data.telegram} className="w-44" content="Let's Talk">
