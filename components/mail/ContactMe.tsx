import { LegacyRef , useRef , RefObject , useState} from 'react';
...
const sendEmail = async () => {
  ...
  console.log(isLoadingForResponse);
  

  return (
    <form ref={formRef} className="" action={sendEmail}>
    <input placeholder="Full Name" type="text" name="user-full-name" className="input-send-email"/>
    <input placeholder="Email" type="email" name="user-email" className="input-send-email"/>
    <input placeholder="Subject" type="text" name="user-subject" className="input-send-email"/>
    <textarea  name="user-message" className=" h-44 input-send-email resize-none overflow-y-auto" ></textarea>
    <DefaultButton isLoading={isLoadingForResponse} content="send" className="w-full">
